...
export default function BakerCalculator() {
  ...
  // Hydrate defaults from server settings
  useEffect(() => {
    if (settings?.defaults) {
      setServings(settings.defaults.servings || 12);
      setDeliveryMiles(settings.defaults.delivery?.miles || 0);
      setRush(settings.defaults.modifiers?.rush || false);
    }
  }, [settings]);
