...
- **AI Lab**: Deterministic AI testing interface with explicit context requirements. Features: context panel with Quote/Lead dropdowns + Brief textarea, AI actions disabled until context selected, automatic draft quote creation from leads via ensureQuoteForLead helper (creates customer + quote if needed), suggest-items returns rationale-enhanced suggestions, all endpoints accept {quoteId?, leadId?, brief?} and return resolvedQuoteId. Integration: "Create Quote from Lead" button in Leads table calls AI suggest-items to auto-create draft quote with suggested items and navigate to quote detail. Endpoints: POST /api/ai/suggest-items, POST /api/ai/summarize-quote, POST /api/ai/generate-contract. No arbitrary behavior - all AI actions require explicit quote or lead context.
